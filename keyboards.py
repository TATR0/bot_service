...
CLIENT_NOTIFY = {
    ...
    "cancelled": "🚫 <b>Заявка отменена.</b>\n\nВы отменили свою заявку. Если передумаете — создайте новую.",
}
...
def admin_menu_keyboard():
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="📋 Мои заявки"), KeyboardButton(text="🔎 Фильтр заявок")],
            [KeyboardButton(text="📊 Статистика"), KeyboardButton(text="👥 Администраторы")],
...
def confirm_leave_keyboard(service_id: str):
    """Подтверждение выхода из сервиса"""
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="✅ Да, покинуть", callback_data=f"leave_confirm:{service_id}"),
